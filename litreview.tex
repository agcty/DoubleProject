...
A more sensible approach for sites with often changing content is \acrfull{ssr}. As opposed to prerendering, \acrlong{ssr} is a lot more sophisticated and complex to set up as it requires a tight coupling to the used framework.

The first is to implement \acrfull{ssr} within a framework itself. This technique allows \acrshort{spa}'s to  It render which renders "[] into HTML strings on the server, 

% Dynamic Rendering
A possible solution which is still relevant today is to "create a static representation of your web site/application and direct crawlers to use it"  was identifed by Fink et al. in 2014 \cite[p.~270]{Fink2014:inbook}.
